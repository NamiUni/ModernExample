...
public final class PaperMessageRenderer<T extends Audience> implements IMessageRenderer<T, String, Component, Component> {

    private final MiniMessage miniMessage;

    @Inject
    public PaperMessageRenderer() {
    ...
    public Component render(
        ...
        if (ModernExample.miniPlaceholdersLoaded()) {
            tagResolver.resolver(MiniPlaceholders.getGlobalPlaceholders());
        }

        if (!(receiver instanceof Player sender && sender.isOnline())) {
            return this.miniMessage.deserialize(intermediateMessage, tagResolver.build());
        }

        final Player senderBukkitPlayer = requireNonNull(Bukkit.getPlayer(sender.getUniqueId()));
